		...
		try {
			...
			if (sessionUser != null) {
				if (sessionUser.getAttribute("user") != null) {
					...
					String reasons= XssParse.parseXSS("I like "+iLikeHobby+" because "+(String) request.getHeader("reasons"));
					...
					if(allHobbies.contains(iLikeHobby)==true){
						...
						out.print("true with new hobby");
						response.setHeader("redirect", "http://localhost:8017/onlinehobby/MakeHobby");
					}
					out.flush();
				} else {
